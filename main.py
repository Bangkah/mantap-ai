...
class OllamaClient:
    ...
    def generate_response(self, model: str, prompt: str) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": "Request timeout. Server mungkin sedang sibuk."}
...
async def index(request: Request):
    if not os.path.exists("templates"):
        return HTMLResponse("""
        <html>
            <head><title>Mantap AI</title></head>
            <body>
                <h1>Mantap AI Backend</h1>
                <p>Backend is running! Please create templates/index.html for frontend.</p>
...
async def chat(
    ...
    if len(prompt) > 10000:
        raise HTTPException(status_code=400, detail="Prompt terlalu panjang (maksimal 10.000 karakter)")
    ...
    if not ollama_client.check_connection():
        raise HTTPException(
            status_code=503, 
            detail="Ollama server tidak tersedia. Pastikan Ollama sedang berjalan."
        )
